...
__version__ = "$Revision: 2587 (standalone version 5/21/2017)$"

# Standalone version 5/21/2017
# Includes newConx version 4/11/2015
# Jim Marshall

import oldnumeric as Numeric
import math, random, time, sys, operator
print "Conx%s" % __version__.split(':')[1][:-1]
        ...
        while line:
            if line.startswith("layer,"):
                ...
                if netType == "cascornetwork":
                    raise Exception, "CascorNetwork unavailable"
...
class Network(object):
                ...
                for connection in self.connections:
                    if connection.active and connection.toLayer.name == layer.name:
                        connection.toLayer.netinput = connection.toLayer.netinput + \
                                                      Numeric.dot(connection.fromLayer.activation,\
        ...
        for connect in reverse(changeConnections):
            if connect.active and connect.toLayer.active and connect.fromLayer.active:
                ...
                connect.fromLayer.error = connect.fromLayer.error + \
                                          Numeric.dot(connect.weight,connect.toLayer.delta)
    ...
    def loadTargetsFromFile(self, filename, cols = None, everyNrows = 1,
        ...
        self.targets = self.loadVectors(filename, cols, everyNrows,
                                        delim, checkEven)
        # verify target patterns match output layer size - added 5/21/2017
        outputSize = sum([len(layer) for layer in self.layers if layer.type == 'Output'])
        for target in self.targets:
            assert len(target) == outputSize, 'target patterns do not match output layer size'
...
# - fixed subtle bug in BackpropNetwork.propagate
# - improved functionality of PGM class
#=================================================================================

import os
from conxgraphics import GraphWin, Line, Point, color_rgb, Text
    ...
    def __init__(self, file=None, values=None, title='', shape=0, scale=0, highlight=None, invert=False):
        ...
        if file is None and values is not None:
            ...
            if pgmType == 'P5':
                ...
                self.raw = [eval(v) for v in f.read().split()]
            for v in self.raw:
...
#                 lines.extend([l1, l2])
    ...
    # NetworkActivationDisplay.update
...
class BackpropNetwork(Network):
    ...
    def __init__(self, name='Backprop Network', verbosity=0):
        ...
        self.testTargets = []
        self.resetLimit = 1
        self.resetEpoch = 10000   # added 4/2/2017
...
#             ad.close()
    ...
    def train(self, sweeps=None, cont=0):
        if sweeps is None:
            pass
            #self.resetEpoch = 10000    # removed this 4/2/2017
        else:
    ...
    def totalError(self):
        ...
        # this forces sweep to go through the dataset patterns in order
        self.setOrderedInputs(1)
        # totalCount = length of one target pattern * total number of target patterns
        tssError, totalCorrect, totalCount, layerData = self.sweep()
        # restore previous values
        self.setLearning(learn)
        self.setOrderedInputs(order)
        self.setInteractive(interact)
    ...
    def evaluate(self):
        ...
        for i in range(len(self.inputs)):
            ...
            # error check added 5/21/2017:
            if correctAnswer == '???':
                print target
                raise ValueError, 'no classification found for target pattern'
    ...
    def saveHiddenReps(self, filename):
        ...
        self.sweep()
        hidden.closeLog()
        # can't use len(self.inputs) in place of self.count here because
        # if network is an SRN, self.inputs will be a list of sequences
        print '%d hidden layer patterns saved in %s' % (self.count, logfile)
        #if 'classify' in dir(self):
        if self.classifications is not None:
...
class SRN(BackpropNetwork):
    ...
    def propagate(self, **args):
        """
        Sets error flags and propagates.
        """
